const Venda = require("../modelsVenda/VendaModels");
...
module.exports = {
    ...
    async create(req, res){
        const {Valor, Marca, Quantidade} = req.body;


        if(!Valor || !Marca || !Quantidade){
    ...
    async update(req,res){
        const{id} = await req.params;
        const {Valor,Marca,Quantidade} = req.body;
        const valor = await Valor.findOne({_id:id});
    ...
    async procura(req,res){
        const{id} = await req.params;
        const vendaProcura = await Venda.findOne({_id:id});
